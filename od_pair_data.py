...
def add_route_complexity(df, nr):
    ...
    df["1_simplest_path_complexity"] = None
    df["1_shortest_path_complexity"] = None
...
def normalize_complexity(df):
    df['shortest_path_complexity_normalized'] = None
    df['simplest_path_complexity_normalized'] = None
    ...
    df['simplest_path_complexity_normalized'] = df['simplest_path_complexity'] / max_complexity
    df['shortest_path_complexity_normalized'] = df['shortest_path_complexity'] / max_complexity

    print("shortest path complexity normalized")
    print(f"max complexity: {df['shortest_path_complexity_normalized'].max()} sum of columns: {df['shortest_path_complexity_normalized'].sum()}, mean: {df['shortest_path_complexity_normalized'].mean()}, median: {df['shortest_path_complexity_normalized'].median()}")
    print("simplest path complexity normalized")
    print(f"max complexity: {df['simplest_path_complexity_normalized'].max()} sum of columns: {df['simplest_path_complexity_normalized'].sum()}, mean: {df['simplest_path_complexity_normalized'].mean()}, median: {df['simplest_path_complexity_normalized'].median()}")
...
def fix_and_combine(od_pair_data):
    ...
    def is_outlier(row):
        return (
            (row['simplest_path_length'] < (Q1['simplest_path_length'] - 1.5 * IQR['simplest_path_length'])) or
            (row['simplest_path_length'] > (Q3['simplest_path_length'] + 1.5 * IQR['simplest_path_length'])) or
    ...
    Q1 = od_pair_data[['simplest_path_complexity', 'shortest_path_complexity']].quantile(0.25)
    Q3 = od_pair_data[['simplest_path_complexity', 'shortest_path_complexity']].quantile(0.75)
    IQR = Q3 - Q1

    def is_outlier(row):
        return (
            (row['simplest_path_complexity'] < (Q1['simplest_path_complexity'] - 1.5 * IQR['simplest_path_complexity'])) or
            (row['simplest_path_complexity'] > (Q3['simplest_path_complexity'] + 1.5 * IQR['simplest_path_complexity'])) or
            (row['shortest_path_complexity'] < (Q1['shortest_path_complexity'] - 1.5 * IQR['shortest_path_complexity'])) or
            (row['shortest_path_complexity'] > (Q3['shortest_path_complexity'] + 1.5 * IQR['shortest_path_complexity']))
        )
    od_pair_data['complexity_outliers'] = od_pair_data.apply(is_outlier, axis=1)
...
if __name__ == "__main__":
    ...
    df1 = pd.read_csv('od_pair_data/local_graphs_randnode1_simplified_drive_B.csv')
    df2 = pd.read_csv('od_pair_data/local_graphs_randnode2_simplified_drive_B.csv')
    df3 = pd.read_csv('od_pair_data/local_graphs_randnode3_simplified_drive_B.csv')
    combined_df = pd.concat([df1,df2,df3], ignore_index=True)
    print(f"columns: {combined_df.columns}")
    combined_df.columns = [re.sub("^[0-9]+_", "", col) for col in combined_df.columns]
    combined_df.columns = [re.sub("^A+_", "", col) for col in combined_df.columns]
    combined_df.columns = [re.sub("^B+_", "", col) for col in combined_df.columns]
    combined_df.columns = [re.sub("^c+_", "", col) for col in combined_df.columns]
    print(f"columns: {combined_df.columns}")
    combined_df = fix_and_combine(combined_df)
    combined_df = normalize_complexity(combined_df)
    combined_df.to_csv('od_pair_data/od_pair_data.csv', index=False)
