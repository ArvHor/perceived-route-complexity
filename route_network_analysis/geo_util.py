...
def get_azimuth(G, point_a, point_b, return_all=False):
    ...
    if isinstance(point_a,int) and isinstance(point_b,int):
        point_a = (G.nodes[point_a]['x'], G.nodes[point_a]['y'])
        point_b = (G.nodes[point_b]['x'], G.nodes[point_b]['y'])


    if not isinstance(point_a, tuple) or not isinstance(point_b, tuple):
        msg = f"Both point_a and point_b must be tuples representing coordinates.{point_a} and {point_b} are not."
        raise ValueError(msg)

    if ox.projection.is_projected(G.graph['crs']):
        lat1, lat2, long1, long2 = point_a[1], point_b[1], point_a[0], point_b[0]
...
def perpendicular_distance(point, line_start, line_end):
    start_point = Point(line_start)
    end_point = Point(line_end)
    point = Point(point)
    line = LineString([start_point, end_point])

    nearest_line_point = line.interpolate(line.project(point))

    distance = ox.distance.great_circle(lat1=point.y, lon1=point.x, lat2=nearest_line_point.y, lon2=nearest_line_point.x)

    return distance


def douglas_peucker(route_linestring_coords, thold):

    dmax = 0.0
    index = 0
    first, last = route_linestring_coords[0], route_linestring_coords[-1]

    # Find the point with max distance from the line between first and last
    for i in range(1, len(route_linestring_coords) - 1):
        d = perpendicular_distance(route_linestring_coords[i], first, last)
        if d > dmax:
            index = i
            dmax = d

    if dmax >= thold:
        recResults1 = douglas_peucker(route_linestring_coords[:index + 1], thold)
        recResults2 = douglas_peucker(route_linestring_coords[index:], thold)

        results = recResults1[:-1] + recResults2
    else:
        results = [route_linestring_coords[0], route_linestring_coords[-1]]
...
def get_bearing_difference(G, origin, intermediate, destination):
    ...
    if isinstance(intermediate, int) or isinstance(origin,np.int64):
        intermediate = (G.nodes[intermediate]['x'], G.nodes[intermediate]['y'])
